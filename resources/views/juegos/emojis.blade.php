...
<div id="contenedor" style="opacity:0;">

  <div id="game" class="fs">

    <div id="hintBtn">Pista</div>
...
new TimelineMax({onStart:populate})
		.set(game, 			  {userSelect:'none', background:'transparent'})//radial-gradient(#333, #000 120%)
		...
		.set('.foundTxt',	{left:0, top:30, width:1024, fontSize:35, textAlign:'center', textContent:'Encuentra los emojis iguales', fontWeight:300, letterSpacing:0.25})
		...
		.to('#contenedor', 0.2, {alpha:1, ease:Power2.easeIn}, 0)
...
function populate() {
    ...
    matched = false;
		msTilHint = 5000;
		TweenMax.set(hintBtn, {autoAlpha:0, textContent:"Pista?", fontSize:25});
...
function updateTime(){
      ...
      alert("Has ganado "+found+" puntos");
			clearInterval(timeInt);
			timeInt = undefined;
			if (found==0) TweenMax.set('.foundTxt', {textContent:'Puntos: 0', fontWeight:500});
			TweenMax.set('.endTxt', {textContent:hint+" Ganar mas puntos? "+hint, fontSize:35});
...
// Browser resize
function doResize() {
  cw = contenedor.clientWidth;
  ch = contenedor.clientHeight;
