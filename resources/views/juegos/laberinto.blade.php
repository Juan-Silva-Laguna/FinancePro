...
<head>
    ...
    <style>
        @import url('https://fonts.googleapis.com/css?family=Inconsolata|Kirang+Haerang');
...
<body>
    <header>
    <h1>Maze Game</h1>
    <p class="scoreboard">won: 0 | lost: 0 | moves: 0 | best: ??</p>
    <p class="timer">[ 0:59 ]</p>
    </header>
    <div class="game">
    <svg width="490" height="490" class="maze"></svg>
    </div>
    <footer>
    <h1>How to play?</h1>
    <p>The goal of the game is to lead the blue dot to the green box using the least amount of moves. Move your cursor over the maze and click on the spot that you want the blue dot to move to. The dot can't move through walls so be sure to choose a clear path for it to follow.</p>
    </footer>
...
<script>
    ...
    static applyPositionAndNeighbours(matrix) {
        ...
        row.forEach((node, x) => {
            ...
            right: row[x + 1] || null,
            unvisitedPositions() {
                return ["top", "left", "bottom", "right"].filter(
                (p) => this[p] && !this[p].visited
                );
            },
            };
        });
    ...
    static make(width, height) {
        ...
        const [x, y] = [width, height].map((num) =>
    ...
    static create(element) {
        ...
        m.innerHTML +=
        '<line class="line" x1="20", y1="20", x2="30", y2="20" style="stroke:rgb(255,0,0);stroke-width:2"/>';
        m.innerHTML +=
        '<circle class="player" cx="0" cy="0" r="8" fill="#03A9F4" stroke="#CFD8DC" stroke-width="2" style="transform: translate(20px, 20px)"/>';
    }
    }
    const util = {
    ...
    doLinesIntersect: ([a, b], [c, d]) => {
        ...
        const s =
        (-s1[1] * (a[0] - c[0]) + s1[0] * (a[1] - c[1])) /
        (-s2[0] * s1[1] + s1[0] * s2[1]);
        const t =
        (s2[0] * (a[1] - c[1]) - s2[1] * (a[0] - c[0])) /
        (-s2[0] * s1[1] + s1[0] * s2[1]);
    ...
    getPointOfIntersection: (
        ...
        return [saX + a * (eaX - saX), saY + a * (eaY - saY)];
    },
    ...
    updateScoreBoard() {
        ...
        this.scoreboard.innerHTML = `won: ${won} | lost: ${lost} | moves: ${moves} | best: ${
        best === Infinity ? "??" : best
        }`;
    }
    ...
    clickHandler(clickEvent) {
        ...
        [this.getPlayerPosition(), localPoint],
        this.walls
        );
        ...
        if (this.score.moves < this.score.best)
            this.score.best = this.score.moves;
        // Start a new game
        this.newGame(true);
        }
    ...
    mouseMoveHandler(event) {
        ...
        if (performance.now() - this._then > 0) {
            ...
            closestIntersection || localPoint,
            ];
